...
public class BaseGoodMovement : MonoBehaviour
{
    ...
    private void handleMovement()
    {
        ...
        if (!onSlope)
        {
            ...
            //Normal gravity if not climbing
            if (!canClimb)
            {
                rb.gravityScale = 2f; // Ensure gravity is normal when not on slope
            }
            // If no horizontal input, decelerate to 0, otherwise accelerate to target speed
            if (movex == 0f)
            {
            ...
            else //if horizontal input, accelerate to target speed
            {
        ...
        }// If on slope and no horizontal input, stick to slope and don't slide down, otherwise move along slope
        ...
        else if (!jumping) //IF ON SLOPE STUFF
        {
